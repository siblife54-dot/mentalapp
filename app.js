(() => {
  ...
  const state = {
    records: [],
    todayDraft: makeEmptyEntry(todayStr()),
    chart: null,
    activeTab: 'today',
    period: '7',
    metric: 'total'
  ...
  function initTelegram() {
    ...
    try {
      ...
      console.warn('Telegram init issue:', error);
    }
  }
    ...
    root.innerHTML = `
      ...
      <section class="section-card" style="display:grid; gap:12px;">
        ...
        <div>
          ...
          <div class="controls" id="metricControls">
  ...
  const storage = {
    async getCloudRecords() {
      ...
      try {
        ...
        const payloads = await Promise.all(keys.map((k) => cloudGet(k)));
  ...
  function mergeRecords(primary, secondary) {
    ...
    [...primary, ...secondary].forEach((entry) => {
      ...
      if (!old || (entry.updatedAt || '') > (old.updatedAt || '')) byDate.set(entry.date, normalizeEntry(entry));
    });
